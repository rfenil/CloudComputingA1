...
export default function BrowseArea() {
  const { trigger: subscribeTrigger } = useBackendMutation<ISubscribeRequest, IResponse<null>>(URLs.post, {
    onSuccess: () => {
  ...
  const {
    ...
    mutate,
  } = useBackendQuery<IResponse<MusicItem[]>>(`${URLs.get}${buildURLSearchParams({ user_id: userId })}`, {
    onSuccess: (res) => {
      if (res?.data) {
    ...
    <Card className="h-fit overflow-auto">
      ...
      <CardContent className="flex flex-wrap gap-4">
        {musicItems.map((item) => {
          ...
          return (
            <div key={item.id} className="flex flex-col border-2 p-4 rounded-lg gap-4 w-[300px]">
