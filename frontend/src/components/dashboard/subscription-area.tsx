...
export default function SubscriptionArea() {
  ...
  const { trigger: unSubscribeTrigger } = useBackendMutation<IUnsubscribeRequest, IResponse<null>>(URLs.post, {
    onSuccess: () => {
  ...
  const unSubscribe = async (user_id: string, song_id: string) => {
    ...
    setUnsubscribingSongs((prev) => [...prev, song_id])

    try {
      await unSubscribeTrigger({ user_id, song_id })
    } finally {
      // Remove loading state for this song when done
      setUnsubscribingSongs((prev) => prev.filter((id) => id !== song_id))
    }
  }
